...
namespace {
struct Sample {std::array<float,3> src{},dst{};std::array<int,8> nodes{};std::array<float,8> weights{};bool holdout=false;};
...
double rmse(const LookLut *lut,const std::vector<Sample> &samples){
 double sum=0;size_t count=0;for(const auto &s:samples)if(s.holdout){auto a=lut?lut->sample(s.src[0],s.src[1],s.src[2]):s.src;
...
LookCalibrationResult fitLookImages(const QImage &baseline,const QImage &reference,const CancelToken &cancel){
 ...
 // A coarse gradient alignment gate prevents learning from unrelated/cropped pictures.
 ...
 const double alignment=cross/std::sqrt(std::max(1e-20,aa*bb));out.report["gradientAlignment"]=alignment;
 if(aa<1e-6||bb<1e-6||alignment<.72)return reject("Reference alignment/texture check failed; use the same uncropped shot (no automatic registration is performed)");
 constexpr int n=17,nodes=n*n*n;auto lut=std::make_shared<LookLut>(*LookLut::identity(n));
 lut->title="Image-specific reference match";
 std::vector<Sample> samples;samples.reserve(size_t(size.width())*size.height());
 std::vector<float> mass(nodes,0),diagonal(nodes,0);std::vector<std::array<float,3>> gradient(nodes);
 for(int y=1;y<size.height()-1;++y)for(int x=1;x<size.width()-1;++x){
  if(cancelled(cancel))return reject("Cancelled");Sample s;s.src=pixel(a,x,y);s.dst=pixel(b,x,y);
  // Hold out whole 16x16 blocks rather than individual adjacent pixels.
  s.holdout=((x/16)+3*(y/16))%5==0;
  std::array<int,3> base{};std::array<float,3> f{};
  for(int c=0;c<3;++c){float q=s.src[c]*(n-1);base[c]=std::min(int(q),n-2);f[c]=q-base[c];}
  int j=0;for(int z=0;z<2;++z)for(int g=0;g<2;++g)for(int r=0;r<2;++r){
   ...
   s.nodes[j]=index;s.weights[j++]=w;if(!s.holdout){mass[index]+=w;diagonal[index]+=w*w;}
  }
  samples.push_back(s);
 }
 ...
 out.report["scope"]="this-image-only-not-camera-characterization";out.report["colorDomain"]="display-srgb";
 ...
 lut->evidence=QJsonObject::fromVariantMap(out.report);lut->evidence["kind"]="image-specific-fit";
...
LookCalibrationResult calibrateLook(const LookCalibrationRequest &request,const CancelToken &cancel){
 ...
 try{
  ...
  const QImage small=request.linearSource.scaled(512,512,Qt::KeepAspectRatio,Qt::SmoothTransformation);
  ...
  if(out.lut){auto l=std::make_shared<LookLut>(*out.lut);l->evidence["provenance"]=QJsonObject::fromVariantMap(request.provenance);
